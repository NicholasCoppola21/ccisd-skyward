...
if (SkywardAccountManager.isError(gradebook)) {
  console.error(gradebook);
  process.exit();
}

console.log(`Here's all of my classes: ${Object.keys(gradebook).join(", ")}`);
    ...
    .map(
      (c) =>
        `Term Grades for ${c.name}: ${c.termGrades
          ...
          .map(
            (a) =>
              `${a.name}: ${a.grade ? a.grade : "N/A"} (${a.term}) Date: ${
                a.dueDate
              }`,
          ) // If a grade isn't inserted yet (*/100), it will be undefined
          .join(", ")}`,
    )
    .join("\n"),
);
